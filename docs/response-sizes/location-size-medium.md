...
curl -s http://localhost:8000/size/medium | jq -r '.'
```
```bash # Respond with headers:
curl -I http://localhost:8000/size/medium
```
+++ Headers
``` #
HTTP/1.1 200 OK
...
```
+++ Response
```text # String "echo" repeated 15,000 times.
...
:::code source="../../config/openresty/conf.d/chamber.conf" title="config/openresty/conf.d/chamber.conf" region="/size/medium" :::